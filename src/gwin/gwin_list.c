...
#if USE_USER_DEFINED_EXTRA_uGFX
int gwinListAddItemWithParam(GHandle gh, const char* text, bool_t useAlloc, uint32_t param){
#else
int gwinListAddItemWithParam(GHandle gh, const char* text, bool_t useAlloc, uint16_t param){
#endif
	ListItem	*newItem;
...
int gwinListAddItem(GHandle gh, const char* text, bool_t useAlloc) {
	return gwinListAddItemWithParam(gh,text, useAlloc,0);
}
...
#if USE_USER_DEFINED_EXTRA_uGFX
uint32_t gwinListItemGetParam(GHandle gh, int item) {
#else
