...
#define SDRAM_DMAx_CHANNEL                DMA_CHANNEL_7
#define SDRAM_DMAx_STREAM                 DMA2_Stream7
#define SDRAM_DMAx_IRQn                   DMA2_Stream7_IRQn
...
{ 
  ...
  sdramHandle.Init.CASLatency         = FMC_SDRAM_CAS_LATENCY_3;//FMC_SDRAM_CAS_LATENCY_2;
...
static HAL_StatusTypeDef _HAL_DMA_Init(DMA_HandleTypeDef *hdma)
{
  ...
  uint32_t tickstart = st_lld_get_counter();\
  /* Check if the DMA Stream is effectively disabled */
  while((hdma->Instance->CR & DMA_SxCR_EN) != RESET)
  {
    /* Check for the Timeout */
    if( TIME_I2MS(st_lld_get_counter() - tickstart) > HAL_TIMEOUT_DMA_ABORT)
    {
      /* Update error code */
  ...
  if(hdma->Init.FIFOMode == DMA_FIFOMODE_ENABLE)
  {
    ...
    tmp |= hdma->Init.FIFOThreshold;//Minor:Salah
  ...
  DMA_Base_Registers *regs = (DMA_Base_Registers *)DMA_CalcBaseAndBitshift(hdma);
...
{  
  ...
  /* Enable GPIOs clock */
  __HAL_RCC_GPIOD_CLK_ENABLE();//__HAL_RCC_GPIOC_CLK_ENABLE();//Changed By Salah
...
//  HAL_GPIO_Init(GPIOC, &gpio_init_structure);
  ...
  NVIC_SetPriority(SDRAM_DMAx_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0x0F, 0));
  /* Enable interrupt */
